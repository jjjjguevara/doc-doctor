...
impl Refinement {
    ...
    pub fn new(value: f64) -> Result<Self, DocDoctorError> {
        if !(Self::MIN..=Self::MAX).contains(&value) {
            return Err(DocDoctorError::RefinementOutOfRange {
                value,
                position: None,
            });
        }
    ...
    /// Create a Refinement without validation (for internal use)
    ///
    /// # Safety
    /// The caller must ensure the value is in the valid range.
    pub fn new_unchecked(value: f64) -> Self {
        debug_assert!(
            (Self::MIN..=Self::MAX).contains(&value),
            "Refinement value {} is out of range",
            value
        );
...
mod tests {
    ...
    #[test]
    fn test_labels() {
        assert_eq!(Refinement::new_unchecked(0.95).label(), "Excellent");
        assert_eq!(Refinement::new_unchecked(0.75).label(), "Good");
        assert_eq!(Refinement::new_unchecked(0.55).label(), "Moderate");
        assert_eq!(Refinement::new_unchecked(0.35).label(), "Weak");
        assert_eq!(Refinement::new_unchecked(0.15).label(), "Poor");
    }

    #[test]
    fn test_serialization() {
        let r = Refinement::new_unchecked(0.75);
